...
sto-rem.utils.formatDate = function (string) {
    if (string && string != '0000-00-00 00:00:00' && string != '-1-11-30 00:00:00' && string != 0) {
        ...
        return date.strftime(sto-rem.config['date_format']);
    }
    else {
        return '&nbsp;';
    }
...
sto-rem.utils.renderBoolean = function (value) {
    return value
        ? String.format('<span class="green">{0}</span>', _('yes'))
        : String.format('<span class="red">{0}</span>', _('no'));
};

sto-rem.utils.getMenu = function (actions, grid, selected) {
    var menu = [];
...
sto-rem.utils.renderActions = function (value, props, row) {
    var res = [];
    ...
    for (var i in row.data.actions) {
        ...
        item = String.format(
            '<li class="{0}"><button class="sto-rem-btn sto-rem-btn-default {1}" action="{2}" title="{3}"></button></li>',
    ...
    return String.format(
        '<ul class="sto-rem-row-actions">{0}</ul>',
        res.join('')
    );
};


sto-rem.utils.userLink = function (value, id, blank) {
    if (!value) {
        return '';
    }
    else if (!id) {
