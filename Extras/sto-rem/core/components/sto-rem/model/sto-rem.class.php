...
class sto-rem
{
    ...
    {
        $this->modx =& $modx;
        $corePath = MODX_CORE_PATH . 'components/sto-rem/';
        $assetsUrl = MODX_ASSETS_URL . 'components/sto-rem/';
        ...
        $this->modx->addPackage('sto-rem', $this->config['modelPath']);
        $this->modx->lexicon->load('sto-rem:default');
        ...
        switch ($ctx) {
            ...
            default:
                if (!defined('MODX_API_MODE') || !MODX_API_MODE) {
                    ...
                    if ($css = $this->modx->getOption('sto-rem_frontend_css')) {
                        $this->modx->regClientCSS(str_replace($config['pl'], $config['vl'], $css));
                    }

                    $config_js = preg_replace(array('/^\n/', '/\t{5}/'), '', '
							sto-rem = {};
							sto-remConfig = ' . $this->modx->toJSON($this->config) . ';
					');


                    $this->modx->regClientStartupScript("<script type=\"text/javascript\">\n" . $config_js . "\n</script>", true);
                    if ($js = trim($this->modx->getOption('sto-rem_frontend_js'))) {
    ...
    public function runProcessor($action = '', $data = array())
    {
        ...
        $processorsPath = !empty($this->config['processorsPath'])
            ...
            : MODX_CORE_PATH . 'components/sto-rem/processors/';
